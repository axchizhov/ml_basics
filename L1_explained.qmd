...
Когда я читал эти рассуждения и смотрел на картинку, то у меня и правда возникло смутное чувство интуитивного понимания. Но, когда я начал разбираться конкретно, это чувство довольно быстро улетучилось.

Ниже я опишу свою попытку разобраться как L1 действительно работает на практике.
Предлагаю вам проследовать за моими рассуждениями. Ну или сразу [прыгнуть до ключевой идеи](#sec-bottom-line).


# Определим L1

Для простоты будет разбирать все на основе линейной регрессии со среднеквадратичной ошибкой (lasso regression). Для более сложных моделей рассуждения, в принципе, будут такими же.

Итак, функция потерь с L1 регуляризацией:
...
Обучение прошло без проблем, модель показывает приличные прогнозы.
...
Модель правильно выцепила один релевантный признак и дала ему большой вес. Однако, ни один вес в ноль не обратился:

```{python}

print(model.weights.flatten())

# plt.bar(range(len(model.weights)), np.log(np.abs((model.weights.flatten()))))
# plt.xlabel('Feature Index')
# plt.ylabel('Log of Absolute Weight')
# plt.title('Weights')
# plt.show()
```

Неужели нас все это время обманывали и Л1 регуляризация не отбирает признаки? Или все-таки что-то мы делаем не так?
