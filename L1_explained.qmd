---
title: "Почему L1 (Lasso) регуляризация отбирает признаки? От теории к практике"
date: "now"
lang: ru
toc: true
format:
...
class LinearRegressionTrueL1:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.epochs):
            ...
            # Update weights and bias
            self.weights -= self.learning_rate * (d_weights + d_l1)
            self.weights = self.clip(self.weights)
            self.bias -= self.learning_rate * d_bias
...
Однако, пользуясь этой техникой не стоит забывать, что исключенные признаки могут быть значимы, а отобранные — относительно бесполезными. Это будет происходить заметно реже, чем обратная ситуация, но вероятность у этого не нулевая.
